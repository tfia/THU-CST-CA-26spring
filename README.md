...
对每个给定的工作集大小，先按固定步长枚举所有访问位置，再将这些位置随机打乱并在数组中串成一个环形链表。测量时沿着这个随机链表做指针追逐读取，并通过平均访问延迟的台阶判断缓存容量边界。

由于指针追逐读取方法中，下一个读取位置是当前读取位置的数据值，因此 CPU 无法预取下一步位置的数据，减少了预取对实验的干扰。
...
在一个远大于 Cache 的大数组上，按给定步长做顺序循环访问，并保证不同步长下总访问次数相同。随着步长增大，同一条 Cache Line 内被重复利用的次数减少，由延迟拐点估计 Cache Line 大小。
...
使用一个 2 倍 Cache Size 的数组，将其划分为 $2^n$ 个块后只选取奇数块参与访问。把这些奇数块的起始地址随机打乱并串成环形链表，再沿链表做指针追逐读取，通过延迟突增的位置判断相联度。
